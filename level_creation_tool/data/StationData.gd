...
@export var tray_food_ids: Array[String] = ["burger"]
...
func from_dict(data: Dictionary) -> void:
	...
	if data.has("tray_food_ids"):
		tray_food_ids.clear()
		for item in data.get("tray_food_ids", []):
			tray_food_ids.append(str(item))
	elif required_food_id != "":
		tray_food_ids = [required_food_id]
